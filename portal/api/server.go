...
func route(server *server, ssoSrv *ssoServer) http.Handler {
	...
	router.Handle("/api/v1/login/auth/callback", fn.Wrap(ssoSrv.AuthCallback)).Methods(http.MethodPost)
	...
	router.Handle(constant.URLKeyExchange, httpAPI.Wrap(server.ExchangeKey)).Methods(http.MethodPost)
	...
	// TODO: refine it
	router.Handle("/api/v1/device/{device_id}", httpAPI.Wrap(server.DeviceDelete)).Methods(http.MethodDelete)
	router.Handle("/api/v1/device/{device_id}", httpAPI.Wrap(server.DeviceUpdate)).Methods(http.MethodPut)
	router.Handle("/api/v1/device/{device_id}/networks", httpAPI.Wrap(server.AddDeviceToNetworks)).Methods(http.MethodPost)
	router.Handle("/api/v1/device/record/online", httpAPI.Wrap(server.DeviceOnlineRecord)).Methods(http.MethodGet)

	router.Handle("/api/v1/networks", httpAPI.Wrap(server.NetworkList)).Methods(http.MethodGet)
	router.Handle("/api/v1/networks/enable", httpAPI.Wrap(server.EnableNetworks)).Methods(http.MethodPost)
	router.Handle("/api/v1/network", httpAPI.Wrap(server.CreateNetwork)).Methods(http.MethodPost)
	router.Handle("/api/v1/network/{network_id}", httpAPI.Wrap(server.Network)).Methods(http.MethodGet)
	...
	router.Handle("/api/v1/network/{network_id}/status", httpAPI.Wrap(server.ChangeNetworkStatus)).Methods(http.MethodPut)
	...
	router.Handle("/api/v1/network/{network_id}/member/{member_id}", httpAPI.Wrap(server.DeleteNetworkUser)).Methods(http.MethodDelete)
	router.Handle("/api/v1/network/{network_id}/member/{member_id}/devices", httpAPI.Wrap(server.NetworkMemberDevices)).Methods(http.MethodGet)
	router.Handle("/api/v1/network/{network_id}/member/{member_id}/status", httpAPI.Wrap(server.ChangeNetworkMemberStatus)).Methods(http.MethodPut)
	router.Handle("/api/v1/network/{network_id}/member/{member_id}/duration", httpAPI.Wrap(server.ChangeNetworkMemberDuration)).Methods(http.MethodPut)
	router.Handle("/api/v1/network/{network_id}/member/{member_id}/role", httpAPI.Wrap(server.ChangeNetworkMemberRole)).Methods(http.MethodPut)
	router.Handle("/api/v1/network/{network_id}/devices", httpAPI.Wrap(server.NetworkDevices)).Methods(http.MethodGet)
	router.Handle("/api/v1/network/{network_id}/device/{device_id}", httpAPI.Wrap(server.DeleteNetworkDevice)).Methods(http.MethodDelete)
	router.Handle("/api/v1/network/{network_id}/device/{device_id}/status", httpAPI.Wrap(server.ChangeNetworkDeviceStatus)).Methods(http.MethodPut)
	router.Handle("/api/v1/network/device/{device_id}/addable", httpAPI.Wrap(server.NetworkAddableForDevice)).Methods(http.MethodGet)

	router.Handle("/api/v1/invitations", httpAPI.Wrap(server.Invitations)).Methods(http.MethodGet)
	router.Handle("/api/v1/invitation/{invitation_id}", httpAPI.Wrap(server.AcceptInvitation)).Methods(http.MethodPost)
	router.Handle("/api/v1/invitation/{invitation_id}", httpAPI.Wrap(server.HandleInvitation)).Methods(http.MethodPut)

	router.Handle("/api/v1/notifications", httpAPI.Wrap(server.Notifications)).Methods(http.MethodGet)
