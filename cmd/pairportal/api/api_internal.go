...
func (s *server) PeerGraph(ctx context.Context) (*protocol.PeerGraphResponse, error) {
	...
	err := db.Tx(func(tx *gorm.DB) error {
		...
		_ = models.NewDeviceQuerySet(tx).UserIDEq(userID).MachineIDEq(jwt.MachineIDFromContext(ctx)).
			GetUpdater().SetLastSeen(time.Now()).Update()
		...
		err = models.NewNetworkDeviceQuerySet(tx).PreloadDevice().NetworkIDIn(networkIDs...).All(&networkDevices)
		if err != nil {
			return err
		}
		for _, d := range networkDevices {
			relayServerIDs[d.Device.RelayServerID] = struct{}{}
			peers = append(peers, protocol.Peer{
				ID:       protocol.PeerID(d.ID),
				UserID:   protocol.UserID(d.UserID),
				Name:     d.Device.Name,
				IPv4:     d.Device.Address,
				ServerID: protocol.ServerID(d.Device.RelayServerID),
			})
			networsByID[d.NetworkID].Peers = append(networsByID[d.NetworkID].Peers, protocol.PeerID(d.ID))
		}
...
func (s *server) Preflight(ctx context.Context, r *http.Request, req *protocol.PreflightRequest) (*protocol.PreflightResponse, error) {
	...
	err := db.Tx(func(tx *gorm.DB) error {
		...
		if deviceNotExists {
			...
			address, err := queryutil.NextIP(tx)
			if err != nil {
				return err
			}
			...
			err := updater.SetLastSeen(time.Now()).Update()
			if err != nil {
				return err
			}
		} else {
		...
		record := &models.DeviceOnlineRecord{
