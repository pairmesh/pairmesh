...
// Vendor represents the current sso provider (github, wechat)
type Vendor int

// the sso provider which is supported by PairMesh service
const (
	GitHub Vendor = iota
	WeChat
	WeChatMobile
...
type Provider interface {
	...
	AuthCodeURL(redirect, port string) string
...
func AuthCodeLinks(redirect string, node string) []Link {
	...
	var ret []Link
	for _, v := range names {
		url := gMgr.provider(v).AuthCodeURL(redirect, node)
		ret = append(ret, Link{
			Name: vendorName[v],
			Link: url,
		})
