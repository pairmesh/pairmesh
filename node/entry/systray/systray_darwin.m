...
- (void)upsertMenuItem:(MenuItem *)item {
  ...
  if (menuItem == NULL) {
    if ([item->siblingId integerValue] > 0) {
      NSMenuItem *siblingItem = [theMenu itemWithTag:[item->siblingId integerValue]];
      NSInteger index = [theMenu indexOfItem:siblingItem];
      menuItem = [theMenu insertItemWithTitle:item->title action:@selector(menuHandler:) keyEquivalent:@"" atIndex: index];
    } else {
...
void upsert_menu_item(int menuId, int parentMenuId, int siblingId, char* title, short disabled, short checked) {
  MenuItem* item = [[MenuItem alloc] initWithId: menuId withParentMenuId: parentMenuId withSiblingId: siblingId withTitle: title withDisabled: disabled withChecked: checked];
  free(title);
